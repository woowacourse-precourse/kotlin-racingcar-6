##구현할 기능
- 경주할 자동차 이름 입력받는다. 쉼표(,)를 기준으로 구분, 5자 이하만 가능
- 시도할 횟수를 입력받는다.
- 자동차가 전진하기 위해서는 pickNumberInRange()으로 무작위 값이 4이상일 경우 전진, 아니면 멈춘다.
- 게임이 끝나면 누가 우승인지 출력한다. 여러 명이면 쉼표(,)를 이용하여 구분
-사용자가 잘못된 값을 입력할 경우 IllegalArgumentException을 발생시킨 후 애플리케이션은 종료되어야 한다.

##예외 처리
자동차 이름을 입력받을 때
-자동차 이름을 5자이상 입력했을 경우

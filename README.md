...
## 🚀 구현할 기능 목록
1. 사용자로부터 n대의 자동차 이름 입력받기
- 쉼표로 이름을 구분한다.
2. m번의 진행 횟수 입력받기
- 사용자로부터 입력을 받고 이를 저장한다.
- 숫자가 아닐 경우 IllegalArgumentException을 발생시킨다.
3. 클래스 Car를 정의
- 클래스 초기화 시에 이름이 5자를 초과할 경우 illegalArgumentException을 발생시킨다.
- 라운드 시작 시에 자동차마다 0~9사이의 랜덤 값을 생성하고 4이상일 경우 전진시킨다.
- 자동차의 이름과 현채 위치를 표현하는 문자열로 바꿔주는 함수를 정의한다.
4. 라운드를 진행한다.
- 진행해야하는 라운드만큼 반복하여 Car의 move함수를 호출하고 출력한다.
5. 우승차를 출력하고 게임을 종료한다.
- 게임이 종료되면 우승차를 출력하고 둘 이상일 경우 쉼표로 구분하여 출력한다.

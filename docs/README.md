...
- [ ] 이미 들어간 자동차의 이름이 들어가지 않도록 한다.
- [x] 위의 예외 상황들에 모두 해당하지 않는 경우, 문자열을 쉼표로 구분하여 List로 변환 후 RacingCar들로 이뤄진 리스트 형태로 반환한다.
...
#### 4.2. 우승자를 출력하는 기능
- [x] 우승 자동차 이름(들)을 모아놓은 MutableList의 원소(들)을 String 형태로 출력한다.
- [x] 우승 자동차가 2대 이상인 경우 쉼표와 공백(, )을 기준으로 구분한다.
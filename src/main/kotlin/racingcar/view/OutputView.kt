...
class OutputView {
    companion object {
        const val RESULT_WINNER_MESSAGE = "최종 우승자 : %s"
        const val INPUT_GUIDE_CAR_NAME_LIST_MESSAGE = "경주할 자동차 이름을 입력하세요.(이름은 쉼표(,) 기준으로 구분)"
        const val INPUT_GUIDE_PLAY_COUNT_MESSAGE = "시도할 횟수는 몇 회인가요?"
        const val PLAY_RESULT_MESSAGE = "실행 결과"
    }
